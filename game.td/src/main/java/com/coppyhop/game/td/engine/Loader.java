 ...
 * Loader
 * 
 * This represents a resource loader and manager. It can load up files to
 * texture objects, parse font files and load them up into BitmapFonts, and in
 * the future it will be used to load sounds, shaders, and music.
 * 
 * @author kyle
 *
 */
public class Loader {
	...
	public static Texture loadTexture(String filename){
		...
		GL11.glTexImage2D(GL11.GL_TEXTURE_2D, 0, 
				GL11.GL_RGBA, tWidth, tHeight, 0, GL11.GL_RGBA,
		...
		try (final BufferedReader br = 
			...
			while ((line = br.readLine()) != null) {
				...
				if(line.startsWith("char ")){
					String[] test = line.split(" ");
					int charId = 0;
					int x=0,y=0,width=0,height=0, xoffset=0, 
							yoffset=0, xadvance=0;
					for(String s:test){
						if(s.startsWith("id=")){
					...
					glyphs.add(new BitmapGlyph(charId, x, y, width, height, 
